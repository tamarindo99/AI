...
public class Puzzle8 {
    ...
    public static void main(String[] args) {
        ...
        Nodo solucion = resolverPuzzleDFS(puzzle, fin);
        if (solucion != null) {
    ...
    private static Nodo resolverPuzzleDFS(byte[][] puzzle, byte[][] fin) {
        Stack<Nodo> pila = new Stack<>();
        Set<Nodo> visitados = new HashSet<>();

        Nodo nodoInicial = new Nodo(puzzle);
        ...
        while (!pila.isEmpty()) {
            ...
            for (byte[][] sucesor : generarSucesores(actual.getDato())) {
                Nodo nodoSucesor = new Nodo(actual, sucesor);

                if (!visitados.contains(nodoSucesor)) {
